...
def parse_optim_states(files):
    ...
    print("partition_count", state_dicts[0]['optimizer_state_dict']["partition_count"])

    # with TP > 1 it behaves normally like normal zero checkpoint
    # with PP > 1 it keeps one group of params in file 1 and 2nd group of params in file 2

    zero_stage = state_dicts[0]['optimizer_state_dict']["zero_stage"]
    world_size = len(state_dicts) # why is this 1 and not 2? state_dicts[0]['optimizer_state_dict']["partition_count"]
    param_shapes = state_dicts[0]["param_shapes"]
    print("param_shapes", param_shapes) # first part

    # the rest is in the other files
    param_shapes = state_dicts[1]["param_shapes"]
    print("param_shapes", param_shapes) # 2nd part
