...
class DebugUnderflowOverflow:
    ...
    **Performance**:
    ...
    Args:
        ...
        print_fh (:obj:`_io.TextIOWrapper`, `optional`, defaults to ``sys.stdout``):
    ...
    def print(self, *args, **kwargs):
        print(*args, **kwargs, file=self.print_fh)
    ...
    def create_frame(self, module, input, output):
        ...
        # # XXX: configurable path prefix
        # file = f"debug-logs/max/{self.rank:0>3d}-{self.module_names[module]}.csv"
        # first_time = False
        # if file not in self.file_handles:
        #     first_time = True
        #     # self.file_handles[file] = open(file, "a")
        #     log_dir = f"debug-logs/tbs/{self.rank:0>3d}-{self.module_names[module]}"
        # fh = self.file_handles[file]
        ...
        # XXX: refactor
        frame_header, *records = self.frame
        ...
        # headers = ["batch_number"]
        # # XXX: just max for now
        # records_max = [str(self.batch_number)]
        for l in records:
            if "not a tensor" in l:
                continue
            segments = l.split()
            if len(segments) != 3:
                continue
        ...
        # print(",".join(records_max), file=fh)
